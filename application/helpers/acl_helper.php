...
{
	function _is_edit_foreign_comment($a)
	{
		return $a->acl_model->acl_post_cr2($a->session->userdata('auth'));
	}
}
if ( ! function_exists('_is_hide_foreign_comment'))
{
	function _is_hide_foreign_comment($a)
	{
		return $a->acl_model->acl_post_cr3($a->session->userdata('auth'));
	}
}
if ( ! function_exists('_is_del_foreign_comment'))
{
	function _is_del_foreign_comment($a)
	{
		return $a->acl_model->acl_post_cr4($a->session->userdata('auth'));
	}
}
